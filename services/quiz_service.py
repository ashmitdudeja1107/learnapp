...
class QuizService:
    ...
    def generate_quiz_questions(self, num_questions: int = 10, difficulty: str = "medium", question_type: str = "multiple_choice") -> Dict[str, Any]:
        ...
        # Convert to the exact format with proper structure
            ...
            for i, question in enumerate(questions, 1):
                ...
                # Get context from RAG service for this question
                    ...
                    logger.error(f"Error formatting question {i}: {str(question_error)}")
                # Continue with other questions instead of failing completely
                    continue
        
        # Ensure we have at least some questions
            if not formatted_questions:
                logger.error("No questions could be formatted successfully")
                return {"questions": []}
        ...
        # Return in the exact format with "questions" wrapper
            ...
            logger.info(f"Generated {len(formatted_questions)} quiz questions in formatted structure")
            return result
